...
function SmartConfig() {
    ...
    const fetchRoles = async () => {
      try {
        const response = await fetch(
          "https://172.20.150.10:8098/smartlogger/roles/get",
  ...
  useEffect(() => {
    try {
      ...
      try {
        ...
        setParsedContent(flattenXml(xmlDoc.documentElement)); // Flatten XML to key-value pairs
        setIsJson(false);
      } catch {
        setParsedContent({});
      }
  ...
  // Convert XML to a flat key-value map
  const flattenXml = (node, parentKey = "") => {
    let obj = {};
    if (node.nodeType === 1) {
      // Element node
      const currentKey = parentKey ? `${parentKey}.${node.nodeName}` : node.nodeName;

      // Skip the root node by checking if parentKey is empty
      if (parentKey === "" && node.parentNode === node.ownerDocument.documentElement) {
        // If it's the root node, reset the currentKey to an empty string
        currentKey = "";
      }

      if (node.childNodes.length === 1 && node.firstChild.nodeType === 3) {
        // Text node (leaf)
        obj[node.nodeName] = node.firstChild.nodeValue.trim(); // Use node.nodeName directly
      } else {
        // Recursively process children
        for (let i = 0; i < node.childNodes.length; i++) {
          const child = node.childNodes[i];
          obj = { ...obj, ...flattenXml(child, currentKey) };
        }
      }
    }
    return obj;
  ...
  const handleRoleChange = (event) => {
    setFormData({
      ...formData,
      roleNames: Array.isArray(event.target.value) ? event.target.value : [], //ensure it is an array
    });
  };
  ...
  const handleConnectionNameChange = (e) => {
    ...
    let fields = connection?.details?.fields || [];
    ...
    // filter only tnx_id and cor_id when wmapigateway is selected
    if (connectionType === "wmapigateway") {
      fields = fields.filter((field) => ["TransactionID", "CorrelationID"].includes(field.field));
    }
    // Update each field's datatype dynamically based on connection type.
    const updatedFields = fields
      .filter((field) => field.field !== "ResourcePath") //resotrict resource path in keys
      .map((field) => ({
  ...
  const filteredData = configData.filter((row) => {
    const searchTerm = searchQuery.toLowerCase();
    return (
      (row.apiName && row.apiName.toLowerCase().includes(searchTerm)) ||
      (row.resourcePath && row.resourcePath.toLowerCase().includes(searchTerm)) ||
      (row.connectionName && row.connectionName.toLowerCase().includes(searchTerm)) ||
      (Array.isArray(row.roleNames) &&
        row.roleNames.join(", ").toLowerCase().includes(searchTerm)) ||
      (row.status && row.status.toLowerCase().includes(searchTerm))
    );
  });

  //function to hgihlight the text
  const highlightText = (text) => {
    if (!searchQuery) return text; // No highlight if there's no search query
          ...
          <MDBox p={3} lineHeight={0}>
            ...
            <DataTable
              table={{
                columns: [
                  {
                    Header: "API Name",
                    accessor: "apiName",
                    Cell: ({ cell: { value } }) => highlightText(value || ""),
                  },
                  {
                  ...
                  },
                  {
                    ...
                    accessor: "roleNames",
                    Cell: ({ cell: { value } }) =>
                      highlightText(Array.isArray(value) ? value.join(", ") : value || ""),
                  },
                  {
                    Header: "Status",
                    accessor: "status",
                ...
                rows: filteredData, // Use the filtered data here
              }}
            />
          </MDBox>
        </Card>
                    ...
                    >
                      ...
                      {formData.connectionType !== "database" && (
                        <TextField
                          ...
                          InputProps={{
                            endAdornment: (
                              formData.connectionType === "wmapigateway" ? index >= 2 : index >= 6
                            ) ? (
