...
function Overview() {
  ...
  const [latPcktLoss, setlatPcktLoss] = useState([]); // Default latPacktLoss
  const [networkInfo, setnetworkInfo] = useState([]); // Default MonitorNetworkAvail
  const [bandwidthHist, setbandwidthHist] = useState([]); // Default bandwidthHist
    ...
    const fetchData = async () => {
      ...
      try {
        const connectionResponse = await fetch(
          `${process.env.REACT_APP_API_URL}/restv2/BInRestInterface.restful.provider:ui/getConnections`,
          {
            method: "GET",
            headers: {
      ...
      // Fetch connection data end

      // Fetch bandwidthHist data Strts
      try {
        const connectionResponse = await fetch(
          `${process.env.REACT_APP_API_URL}/restv2/BInRestInterface.restful.provider:ui/getBandwidthHist`,
          {
            method: "GET",
            headers: {
              Authorization: authHeader,
        ...
        const connectionData = await connectionResponse.json();
        const connectionRows = connectionData.bandwidth.map((connection) => ({
          hostIP: connection.hostIP,
          deviceIP: connection.deviceIP,
          send_rate: connection.send_rate,
          receive_rate: connection.receive_rate,
          total_rate: connection.total_rate,
          peak_rate_sent: connection.peak_rate_sent,
          peak_rate_received: connection.peak_rate_received,
          cumulative_sent: connection.cumulative_sent,
          cumulative_received: connection.cumulative_received,
          latestUpdateTimestamp: connection.latestUpdateTimestamp,
        }));
        setbandwidthHist(connectionRows);
      } catch (error) {
        console.error("Error fetching connection data:", error);
      }
      // Fetch bandwidthHist data end

      //Fetch LatPacketLostHist
      try {
        const connectionResponse = await fetch(
          `${process.env.REACT_APP_API_URL}/restv2/BInRestInterface.restful.provider:ui/getLatPacktLossHist`,
          {
            method: "GET",
            headers: {
              Authorization: authHeader,
        ...
        const connectionData = await connectionResponse.json();
        const connectionRows = connectionData.getLatPacktLossOutput.map((connection) => ({
          ipAddress: connection.ipAddress,
          pcktsSent: connection.pcktsSent,
          pcktsRcvd: connection.pcktsRcvd,
          pcktsLost: connection.pcktsLost,
          minRtt: connection.minRtt,
          maxRtt: connection.maxRtt,
          avgRtt: connection.avgRtt,
          latestUpdateTimestamp: connection.latestUpdateTimestamp,
        }));
        setlatPcktLoss(connectionRows);
      } catch (error) {
        console.error("Error fetching connection data:", error);
      }
      //Fetch Lat Packets history Ends

      //Fetch Network Info History start
      try {
        const connectionResponse = await fetch(
          `${process.env.REACT_APP_API_URL}/restv2/BInRestInterface.restful.provider:ui/getNetworkInfoHist`,
          {
            method: "GET",
            headers: {
              Authorization: authHeader,
        ...
        const connectionData = await connectionResponse.json();
        const connectionRows = connectionData.getNetworkInfoHist.map((connection) => ({
          ip_address: connection.ip_address,
          status: connection.status,
          latest_updated_timestamp: connection.latest_updated_timestamp,
        }));
        setnetworkInfo(connectionRows);
      } catch (error) {
        console.error("Error fetching connection data:", error);
      }
      //Fetch Network Info history Ends
          ...
          <Grid container spacing={3}>
            ...
            <Grid item xs={12} sm={6} md={6}>
              <Card>
                <MDBox p={3}>
                  <MDTypography variant="h6" fontWeight="medium">
                    Latest Packets Loss History
                  </MDTypography>
                  <DataTable
                    table={{
                      columns: [
                        { Header: "IP Address", accessor: "ipAddress" },
                        { Header: "Packets Sent", accessor: "pcktsSent" },
                        { Header: "Packets Received", accessor: "pcktsRcvd" },
                        { Header: "Packets Lost", accessor: "pcktsLost" },
                        { Header: "Min Rtt", accessor: "minRtt" },
                        { Header: "Max Rtt", accessor: "maxRtt" },
                        { Header: "Avg Rtt", accessor: "avgRtt" },
                        { Header: "LAST UPDATED", accessor: "latestUpdateTimestamp" },
                      ],
                      rows: latPcktLoss,
                    }}
                  ...
                  <DataTable
                    table={{
                      columns: [
                        ...
                        { Header: "LAST UPDATED", accessor: "latest_updated_timestamp" },
                      ],
                      rows: networkInfo,
                    }}
                    entriesPerPage={5}
              ...
              <Card>
                <MDBox p={3}>
                  ...
                  <DataTable
                    table={{
                      columns: [
                        { Header: "host IP", accessor: "hostIP" },
                        { Header: "device IP", accessor: "deviceIP" },
                        { Header: "send rate", accessor: "send_rate" },
                        { Header: "receive rate", accessor: "receive_rate" },
                        { Header: "total ratetotal rate", accessor: "total_rate" },
                        { Header: "peak rate sent", accessor: "peak_rate_sent" },
                        { Header: "peak rate received", accessor: "peak_rate_received" },
                        { Header: "cumulative sent", accessor: "cumulative_sent" },
                        { Header: "cumulative received", accessor: "cumulative_received" },
                        { Header: "latest updated", accessor: "latestUpdateTimestamp" },
                      ],
                      rows: bandwidthHist,
                    }}
                    entriesPerPage={5}
