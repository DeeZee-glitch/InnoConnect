...
function Overview() {
  ...
  //
  const [resourceList, setResourceList] = useState([]); // Store resource names
  const [selectedResource, setSelectedResource] = useState(""); // Track selected resource
  //
  ...
  //
  // State for storing resource names
  const [resources, setResources] = useState([]);

  // Fetch resource names from API
  useEffect(() => {
    const fetchResources = async () => {
      try {
        ...
        const data = await response.json();
        if (data.resourcesName) {
          setResources(data.resourcesName.map((item) => item.resourceName));
        } else {
          console.warn("API response does not contain 'resourcesName'.", data);
        }
      } catch (error) {
    ...
    fetchResources();
  }, []); // Ensure it runs once on component mount
  //
  return (
    <DashboardLayout>
      <DashboardNavbar />
            ...
            {[
              ...
              <Grid item xs={12} md={6} lg={4} key={index}>
                <MDBox position="relative">
                  ...
                  {/* Doughnut Chart with Dropdown Inside */}
                  <DefaultDoughnutChart
                    ...
                    description={
                      <MDBox display="flex" flexDirection="column" alignItems="center">
                        ...
                        {/* <FormControl fullWidth sx={{ mt: 1, width: "80%" }}>
