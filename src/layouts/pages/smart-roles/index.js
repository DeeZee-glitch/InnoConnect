...
function SmartRoles() {
  ...
  const fetchAssignedUsers = async (roleName) => {
    try {
      const response = await fetch(
        ...
        `http://192.168.217.129:5555/restv2/InnoConnect.adminPortal.restful.roleManagement:RoleManagement/getAssignedUsers/${roleName}`,
        {
          method: "GET",
          headers: {
  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `http://192.168.217.129:5555/restv2/InnoConnect.adminPortal.restful.roleManagement:RoleManagement/username/${encodeURIComponent(searchQuery)}`,
        {
          method: "GET", // Use GET for search requests
          headers: {
            "Content-Type": "application/json",
            "Security-Token": `${token}`, // Bearer token for authentication
          },
        }
      );
  ...
  // excluding the roles
  const excludedRoles = ["offline_access", "uma_authorization", "default-roles-innoconnect"];
  ...
  const fetchConfigData = async () => {
    try {
      const response = await fetch("http://192.168.217.129:5555/restv2/InnoConnect.adminPortal.restful.roleManagement:RoleManagement/getRoles", {
        method: "get",
        headers: {
  ...
  const handleAssignUser = async () => {
    ...
    const userId = selectedUser.id; // selected user's ID
    const roleName = selectedRole.name; // selected role name to assign
    const roleId = selectedRole.id; // selected role id for mapping to suer

    try {
      const response = await fetch(
        `http://192.168.217.129:5555/restv2/InnoConnect.adminPortal.restful.roleManagement:RoleManagement/addUserToRole/${userId}`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
  ...
  const handleSubmit = async (e) => {
    ...
    let apiUrl = `http://192.168.217.129:5555/restv2/InnoConnect.adminPortal.restful.roleManagement:RoleManagement/addRole`;

    let method = isEditMode ? "PUT" : "POST"; // Use PUT if in edit mode, otherwise POST
    if (isEditMode && currentEditRow && currentEditRow.name) {
      apiUrl = `http://192.168.217.129:5555/restv2/InnoConnect.adminPortal.restful.roleManagement:RoleManagement/editRole/${encodeURIComponent(currentEditRow.name)}`; // Append uniqueId for PATCH requests
    }
  ...
  const handleDeleteConfirm = async () => {
    ...
    let apiUrl = `http://192.168.217.129:5555/restv2/InnoConnect.adminPortal.restful.roleManagement:RoleManagement/deleteRole/${deleteRow.name}`;
    try {
      const response = await fetch(apiUrl, {
        method: "DELETE",
        headers: {
              ...
              <MDBox display="flex" flexDirection="column" gap={2}>
                {assignedUsers.map((user) => (
                  ...
                  >
                    ...
                    {/* Optional: Display additional user info */}
            ...
            <DataTable
              table={{
                columns: [
                  ...
                  //           </Tooltip>
                  //           <span style={{ color: "green", marginLeft: 8 }}>True</span>
                  //         </>
                  //       ) : (
                  //         <>
                  //           <Tooltip>
                  ...
                  //           </Tooltip>
                  //           <span style={{ color: "grey", marginLeft: 8 }}>False</span>
                  //         </>
                  //       )}
                  //     </div>
                  //   ),
                      ...
                      <MDBox display="flex">
                        ...
                        <Tooltip title="Edit Role">
                          <IconButton
                            ...
                            disabled={excludedRoles.includes(row.original.name)} // Disable if it's a default role
                          >
                            <EditIcon />
                          </IconButton>
                        </Tooltip>
                ...
                rows: filteredRoles || [], // Pass the filtered roles to the DataTable
              }}
            />
          </MDBox>
        </Card>
      ...
      <Dialog open={openDialog} onClose={handleCloseDialog}>
        ...
        <DialogActions>
          ...
          <MDButton
            ...
            disabled={isSubmitting || !formData.name} // Disable if role name is empty
            variant="gradient"
          >
      ...
      <Dialog open={openAssignDialog} onClose={handleAssignDialogClose}>
        ...
        <DialogContent sx={{ padding: "20px", minWidth: "570px" }}>
          <TextField
            ...
            onChange={(e) => {
              ...
              handleSearch(); // Trigger the search function on input change
            }}
            ...
            >
              {searchResults.map((user) => (
                <div
                  ...
                  onClick={() => handleUserSelect(user)} // Assign the user when clicked
                  onMouseEnter={(e) => (e.currentTarget.style.backgroundColor = "#e7f3ff")} // Hover effect
        ...
        <DialogActions>
          ...
          <MDButton
            variant="contained"
            color="primary"
            onClick={handleAssignUser} // Trigger the API call only here
            disabled={!selectedUser || !selectedRole} // Disable the button if no user or role is selected
          >
            Assign User
          </MDButton>
        </DialogActions>
