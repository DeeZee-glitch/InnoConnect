...
function DefaultDoughnutChart({ icon, title, description, height, chart }) {
  // Safe fallback for chart object properties
  const { labels = [], datasets = [], cutout = "50%" } = chart || {};

  // Generate chart data and options
  const { data, options } = configs(labels, datasets, cutout);
...
DefaultDoughnutChart.defaultProps = {
  ...
  description: "",
  height: "19.125rem",
  chart: {}, // Default to an empty object if chart prop is not provided
};
